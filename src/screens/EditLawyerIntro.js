...
const handleSubmit = async () => {
 ...
 if (!validate()) {
  ...
  try {
    ...
    // const img = imageData.assets[0];
    // const reference = storage().ref(img.fileName);
    // await reference.putFile(img.uri);
    // const imageUrl = await reference.getDownloadURL();

    const profilePicBase64 = imageData ? imageData.base64 : "";
...
profilePic: profilePicBase64,
  isPicEdit: !!imageData
    ...
    // 📡 POST to API
    const response = await fetch(
      "https://loving-turing.91-239-146-172.plesk.page/api/LawyerPostApi/EditLawyerIntro",
      {
        method: "PUT",
...
const isFormValid =
  ...
  return (
    <SafeAreaView style={styles.container}>
        <ScrollView>               
   <Text style={styles.txt}>Edit Your Information</Text>
    ...
    <CustomInput plc='First Name' editable={true} keyboardType='default' value={firstName} onChangeText={setFirstName} />
    {/* {errors.firstName && <Text style={styles.error}>{errors.firstName}</Text>} */}
     <Text style={styles.txte}>Last Name</Text>
    <CustomInput plc='Last Name' editable={true} keyboardType='default' value={lastName} onChangeText={setLastName} />
...
export default EditLawyerIntro
